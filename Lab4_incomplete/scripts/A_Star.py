...
def GetPath (gridCellsMap, start_pos, goal_pos):
	...
	trans_start, trans_goal = translatePoints(gridCellsMap, start_pos, goal_pos)
	#Set up Queue and nodes for the path based off of the start and goal pos
	parents, costs, currNode = goalSearch(gridCellsMap, trans_start, trans_goal)
	path = Path()
	poseStampedList = []
	i = 0
	pathList = []
...
def goalSearch (gridCellsMap, start_pos, goal_pos):
	...
	while not frontier.empty():
		...
		if currNode not in visitedList:
			...
			frontiers = GCfromList(frontierList)
			pub_frontier.publish(frontiers)
			visited = GCfromList(visitedList)
